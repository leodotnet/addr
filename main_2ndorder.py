...
if options.model == 'LSTM':
    ...
    # B_label_list = ['B-' + x.lower() for x in label_list]
    # I_label_list = ['I-' + x.lower() for x in label_list]
    #
    # label_list = []
    # for l1 in B_label_list:
    #     for l2 in I_label_list:
    #         l_o2 = l1 + ':' + l2

    label_list = []
    for filename in [options.train, options.dev, options.test]:
        f = open(os.path.join(options.data, filename), 'r', encoding='utf-8')
        for line in f:
            line = line.strip()
            if len(line) == 0:
                continue
            fields = line.split()
            if fields[-1] not in label_list:
    ...
    for l1 in label_list:
        for l2 in label_list:
            l_o2 = l1.lower() + ":" + l2.lower()
            if l_o2 not in label_list:
                label_list_order2.append(l_o2)

        l_o2 = l1.lower() + ":" + "<end>"
        if l_o2 not in label_list:
            label_list_order2.append(l_o2)

    label_list = label_list_order2
    vocab_label = Vocab.from_list(label_list)

print('label_list:',label_list)
reader = Reader(vocab_char, vocab_label, vocab_action, options)

train = reader.read_file(os.path.join(options.data, options.train), 'train', 2)
dev = reader.read_file(os.path.join(options.data, options.dev),'eval', 2)
test = reader.read_file(os.path.join(options.data, options.test),'eval', 2)
